...
static void makeMenu(); // Avoid compiler error
...
int deletedObjects = 0;  // How many objects have been deleted
int currObject = -1;    // The current object
int toolObj = -1;       // The object currently being modified

#define NUM_PERMANENT_OBJS 3 // Ground, light 1, light 2
...
static void addObject(int id) {
    ...
    // *** Task J
    makeMenu(); // Need to update menu to see new object in list
}
...
static void deleteObject(int objID) {
    ...
    currObject = -1;
    deletedObjects++;
    makeMenu(); // Update menu to get rid of this from the object list
}
...
static void cloneObject(int objID) {
    if (nObjects < maxObjects) {
        ...
        currObject = nObjects; // correct index for currObject (1 behind)
        toolObj = currObject;
        nObjects++;
...
void init(void) {
    ...
    sceneObjs[2].scale = 0.2; // Mak slightly bigger to differentiate from light 1
    sceneObjs[2].texId = 0;
...
void drawMesh(SceneObject sceneObj) {
    ...
    // *** Task B
    //
    // define the rotation matrix for a scene object
    // Rotation order Z -> Y -> X
    //     - need to negate the rotate_X argument to imitate the example video
    mat4 rotateXYZ = RotateX(-sceneObj.angles[0]) *
                     RotateY( sceneObj.angles[1]) *
                     RotateZ( sceneObj.angles[2]);

    // apply the rotation matrix to the model view matrix
...
void display(void) {
    ...
    // *** Task A
    //
    // Use rotate functions from mat.h to create rotation matrix
    // Rotation order:
    //   - 1. about y
    //   - 2. about x
    ...
    //
    // Apply rotation then translation to create view matrix
    ...
    // *** Task I
    //
    // Rename lightPosition to lightPosition1
    SceneObject lightObj1 = sceneObjs[1];
    vec4 lightPosition1 = view * lightObj1.loc;
    //
    // Set second light position to be affected only by camera rotation.
    SceneObject lightObj2 = sceneObjs[2];
    vec4 lightPosition2 = rotateXY * lightObj2.loc;
    //
    // Now need to add extra variables to fragment shader
    // Task I ***
    ...
    // *** Task H
    //
    // Need brightness from light to increase value of specular to help tend
    // fragment color tend towards white
    //
    glUniform1f(
        glGetUniformLocation(shaderProgram, "LightBrightness1"),
    ...
    // *** Task I
    //
    // Pass position and brightness of the 2nd, directional, light
    glUniform4fv(glGetUniformLocation(shaderProgram, "LightPosition2"),
                 1, lightPosition2);
    CheckError();

    glUniform1f(
        glGetUniformLocation(shaderProgram, "LightBrightness2"),
...
//
// Left button clicked:
//      adjust sceneObj ambient by mouse horizontal movement
//      adjust sceneObj diffuse by mouse vertical movement
...
//      adjust sceneObj specular by mouse horizontal movement
//      adjust sceneObj shine by mouse vertical movement
...
static void lightMenu(int id) {
    int LIGHT_2_MENU_ID = 80; // from skeleton code
    int LIGHT_2_OBJ_ID = 2;
    ...
    // *** Task I
    // Allow movement of second light
    } else if (id == LIGHT_2_MENU_ID) {
        toolObj = LIGHT_2_OBJ_ID;
        setToolCallbacks(adjustLocXZ, camRotZ(),
                         adjustBrightnessY, mat2(1.0, 0.0, 0.0, 10.0));
    //
    // Task I ***
...
static void materialMenu(int id) {
    ...
    // From what was defined in the skeleton code
    int AMB_DIF_SPEC_SHN_MENU_ID = 20; 

    // 2x2 identity matrix
    mat2 ident_2 = mat2(1, 0, 0, 1);
...
static void selectActiveObjectMenu(int id) {
    int activeObjID = id - 100; // index of the object in the sceneObjects array
    toolObj = activeObjID;
    currObject = activeObjID;
    makeMenu();
}
...
static void makeMenu() {
    ...
    // *** Task C
    //
    // Remove unimplemented from menu name
    ...
    int OBJ_NAME_LEN = 128; // same as gnatidread

    for (int i = NUM_PERMANENT_OBJS; i < nObjects; i++) {
    ...
    int numObjectsInScene = nObjects - deletedObjects - NUM_PERMANENT_OBJS;
    if (numObjectsInScene > 0) {
        glutAddSubMenu("Activate Object", selectActiveObjectMenuId);
    ...
    if (currObject != -1) {
        if (currObject > NUM_PERMANENT_OBJS - 1) {
            // don't want to delete/clone ground or lights
            glutAddMenuEntry("Delete object", 51);
            glutAddMenuEntry("Clone Object", 52);
        }
...
void reshape(int width, int height) {
    ...
    // *** Task D 
    //
    // decreasing this value will allow us to get closer to scene objects
    // without triangles clipping
    ...
    // *** Task E
    //
    // define a perspective transformation matrix for the camera
    //
